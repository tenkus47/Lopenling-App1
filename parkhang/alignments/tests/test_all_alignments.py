from alignments.models import (ImageAlignment, Imagegroup, TextAlignment,
                               Video, VideoAlignment)
...
class AllAlignmentsTestCase(APITestCase):
    ...
    def setUpTestData(cls) -> None:
        cls.source, _ = Text.objects.get_or_create(
            name=cls.text_name
        )
        cls.text_target, _ = Text.objects.get_or_create(
            name=cls.text_name
        )
    ...
    def test_get_source_alignments(self):
        ...
        self.assertEqual(response.data["alignments"]["text"][0]["target"], self.text_target.pk)
        self.assertEqual(response.data["alignments"]["image"][0]["target"], self.image_target.pk)
        self.assertEqual(response.data["alignments"]["video"][0]["target"], self.video_target.pk)

        self.assertEqual(response.data["alignments"]["text"][0]["alignment"], self.text_alignment.pk)
        self.assertEqual(response.data["alignments"]["image"][0]["alignment"], self.image_alignment.pk)
        self.assertEqual(response.data["alignments"]["video"][0]["alignment"], self.video_alignment.pk)
