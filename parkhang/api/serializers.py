...
class WitnessSerializer(serializers.ModelSerializer):
    ...
    is_base = serializers.SlugRelatedField(
        source="source", slug_field="is_base", read_only=True
    )
    is_working = serializers.SlugRelatedField(
        source="source", slug_field="is_working", read_only=True
    )
...
class AnnotationSerializer(serializers.ModelSerializer):
    witness = serializers.PrimaryKeyRelatedField(queryset=Witness.objects)
    creator_witness = serializers.PrimaryKeyRelatedField(
        queryset=Witness.objects, allow_null=True
    )
    creator_user = serializers.PrimaryKeyRelatedField(
        queryset=User.objects, allow_null=True
    )
    original = serializers.SlugRelatedField(
        slug_field="unique_id", queryset=Annotation.objects, allow_null=True
    )
...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    username = serializers.CharField(
        source="annotation.creator_user.sso_username", default=""
    )
